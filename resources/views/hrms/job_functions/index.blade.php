...
<div class="row">
    <div class="col-12" style="margin-bottom: 20px;">
        <a class="btn btn-primary" href="{{ route('departments.units.job_functions.create', [$department->slug(), $unit->slug()]) }}"><i class="fas fa-plus"></i> New Job Function</a>
                        ...
                        <table id="myTable1" class="display-1 table table-condensed table-hover table-striped">
                            <thead>
                                <tr class="text-center">
                                    ...
                                    <th width="55%"><strong>JOB DESCRIPTION</strong></th>
                                    <th width="10%">&nbsp;</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($job_functions as $job_function)
                                    @if ($job_function->active)
                                <tr>
                                    <td>{{ $job_function->name }}</td>
                                    <td>{{ $job_function->description }}</td>
                                    <td class="text-center">
                                        <a title="Edit" href="{{ route('departments.units.job_functions.edit', [$department->slug(), $unit->slug(), $job_function->slug()]) }}"><i class="fas fa-edit"></i></a>&nbsp;&nbsp;
                                        <a title="Trash" href="{{ route('departments.units.job_functions.disable', [$department->slug(), $unit->slug(), $job_function->slug()]) }}" onclick="return confirmDisable()"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                    @endif
                                @endforeach
                        ...
                        <table id="myTable2" class="display-1 table table-condensed table-hover table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $job_function->description }}</td>
                                    <td class="text-center">
                                        <a title="Restore" href="{{ route('departments.units.job_functions.enable', [$department->slug(), $unit->slug(), $job_function->slug()]) }}"><i class="fas fa-undo"></i></a>
                                    </td>
                                </tr>
                                    @endif
                                @endforeach
